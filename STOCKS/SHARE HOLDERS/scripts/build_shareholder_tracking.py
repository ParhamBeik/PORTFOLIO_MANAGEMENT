...
TRACKING_START_DATE = "1404-07-01"  # Baseline date (inclusive) for ledger columns

FETCH_DIR = DATA_DIR / "FETCH_SHAREHOLDER"
...
class ShareholderTrackingManager:
    def __init__(self):
        self.stocks_map: dict[str, str] = {}
        self.holidays: list[str] = []
        self.trading_dates: list[jdatetime.date] = []
            ...
            for symbol_dir in industry_dir.iterdir():
                ...
                for file_path in symbol_dir.glob("*.json"):
                    ...
                    if not date_pattern.match(date_str):
                        continue
                    if date_str in df.columns and df.at[symbol, date_str] != S_SUCCESS:
...
def main():
    print("=" * 70)
    print("📊 SHAREHOLDER TRACKING MANAGER")
    print("=" * 70)

    tracker = ShareholderTrackingManager()
